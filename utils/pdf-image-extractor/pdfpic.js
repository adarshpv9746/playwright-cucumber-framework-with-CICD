...
// Function to generate a unique hash for an image data
...
rimraf('./pdf-images/*.{jpg,png}', async (err) => {
  if (err) console.error(err)
    ...
    const savedImageHashes = new Set() // To track hashes of saved images
    let idx = 0
    for (const img of imagesInDoc) {
      if (!img.isAlphaLayer) {
        ...
        if (!savedImageHashes.has(imageHash)) {
          fs.writeFileSync(`${imagesFolder}/out${idx + 1}.png`, imageData)
